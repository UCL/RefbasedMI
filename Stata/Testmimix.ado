...
*km
 save "N:\Documents\Github\mimix\preimputemvn`i'.dta" ,replace	
 *save "N:\Users\rmjlkm0\Documents\Github\mimix\preimputemvn`i'.dta" ,replace	
*km
			...
			*USE DRAWS OF MEAN AND COVARIANCE MATRICES FROM mi impute IN CONSTRUCTION DEPENDING ON SPECIFIED METHOD	
				...
				*Interim MAR: IF interim IS USED, REGARDLESS OF MNAR METHOD ADD IN mata_MARMeans		
				...
				if `meth' == 3	{
					*J2R 
					...
					mata: cc = cc'
					*km
					*km di "miss_count = " `miss_count'
					*km di "counter = " `counter'
					forvalues b=1/`miss_count'{
						forvalues c=1/`counter' {
					...
					mata: Sigma = mata_VAR_group`refer'_imp`imp'
					*km
					*km di "list required = " `list_required'
					if "`list_required'" != "" {
                ...
                *FOR CASES WITH NO OBSERVED OUTCOMES:                              
				if `nonmiss_count'==0 {
					...
					*km
					*km di "mata_S_nonmiss="
				*km	mata:mata_S_nonmiss
				*km	di "m1 m2 t raw1"
					*mata:m1
					*mata:m2
					*mata:t
				...
				*km check counter, miss_count same as R
				   *km di  "checking counter ,miss_count for inverse norm gen = " `counter' " " `miss_count'
					mata: Z = invnormal(uniform(`counter',`miss_count'))
					...
					*km 22/12mata:Sigma_new=(Sigma_new\Sigma)
...
save "N:\Documents\Github\mimix\mata_all_newCRI10m.dta"  ,replace
keep if id==5456
*keep if id==5333
...
univar fev2 fev4 fev8 fev12 base, dec(3)
restore
save "N:\Documents\Github\mimix\mata_all_newCIR10000m.dta"  ,replace
