...
#Note must be repeated M times same as stata mi impute mvn (mi set)
...
#summary(emNorm(cbind(fev1,fev2,fev3,fev4,base) ~1,data=preimputemvn1))
  emResult1<-emNorm(cbind(fev1,fev2,fev3,fev4,base) ~1,data=preimputemvn1,prior = "ridge",prior.df=0.5)
...
#save the 1000 iterations
save (res1sigma.list,file = "kmres1sigma.RData")
save (res2sigma.list,file = "kmres2sigma.RData")
save (res1beta.list,file = "kmres1beta.RData")
save (res2beta.list,file = "kmres2beta.RData")
...
#need subroutine to find mimix_group?
...
# having found M (1000) parameter vectors start here
#need use mat_Betas and mat_Sigmas
...
#hard code for now
# have to come back to this, maybe add names when not null 
names(mata_all_new) <-names(mata_new2)

#hard code for pattren =11 as 3 missing fields 
mata_U <- matrix(nrow=1,ncol=3) #empty matrix
mata_U<-mata_U[-c(1:nrow(mata_U)),]
names(mata_U)<-names(mata_U)
mata_Z <- matrix(nrow=1,ncol=3)  #empty matrix
mata_Z<-mata_Z[-c(1:nrow(mata_Z)),]
names(mata_Z)<-names(mata_Z)
pat11_t_mimix <-matrix(nrow=1,ncol=3)
pat11_t_mimix<-pat11_t_mimix[-c(1:nrow(pat11_t_mimix)),]
pat11_conds <-matrix(nrow=1,ncol=3)
pat11_conds <-pat11_conds[-c(1:nrow(pat11_conds)),]
#check whether ref =1 is working as intended
...
#introduce M imputation iteration
#M<-1000
#for (m in 1:M) {
#  print("m = ")
#  print(m)
#  testmat_Betas<- c(res1beta.list[m],res2beta.list[m])

# loop for one pattern only, may as well make it the last as only 3 records.  
#for ( i in  1:nrow(mimix_group)) 
i <- nrow(mimix_group)
{
  ...
  SigmaRef <- Reduce(rbind,res1sigma.list[m])
  ...
  mata_means_trt <-testmat_Betas[trt_gp] 
  mata_means_ref <-testmat_Betas[refer]
  ...
  # this could be the error, amended 16/10
  #imp <-1
  imp<-M
  ...
  # recursively build  mata_all_new 
  mata_all_new <-bind_rows(mata_all_new,mata_new2)
  
  #similarly recursively build U and Z, rbind gives different data structure so back to bind_rows
  #names(mata_U) <- names(U)
  #easiest way is use c() operator 
  mata_U <-c(mata_U,U)
  mata_Z <-c(mata_Z,Z) 
  pat11_t_mimix <-c(pat11_t_mimix,t_mimix)
  pat11_conds <-c(pat11_conds,conds)
  #print(mata_all_new)
 }
} # FINISH LoopING  to here k 1:M
test <- matrix(mata_U,ncol=3)
# mata_U is not the same pattern! matrix changes every 3rd line but doesnt matter as symettrised in analysis?
# analyse the non zero rows


save (mata_all_new,file = "kmmata_all_new.RData")
mata_all_5456new <- filter(mata_all_new,id == "5456") 
mean(matta_all_5456$fev3)
#calc for  t test
mn1 <-1.619
sd1 <- 0.45
mn2 <- 1.626
...
library(rcompanion)
groupwiseMean(fev2~1,data=mata_all_5456,conf=0.95,digits = 3)
t.test(mata_all_5604$fev6,conf.level = 0.95)
        
#DF
DegF <- (sd1^2/n1+sd2^2)^2/(((sd1^2/n1)^2/(n1-1))+((sd2^2/n2)^2/(n2-1)))