...
analyselist(5017,mimixout,varlist)
analyselist(5127,mimixout,varlist)
...
impdatasetJ2Rdelta<-(mimix(data=asthma,covar=c(base),depvar=fev,treatvar=treat,idvar=id,timevar=time,M=10,refernce=1,method=J2R,seed=101,delta=c(1,5,2,3),K0=1,K1=1 ) )
...
fit<-with(as.mids(impacupCausal), lm(head.12~treat+head_base))
summary(pool(fit))


##### causa; model is independent on reference !!??] 
...
impantiIndivDt <- mimix("antidepressant",c("basval","PATIENT.SEX"),"HAMD17.TOTAL","TREATMENT.NAME","PATIENT.NUMBER","VISIT.NUMBER",2,NULL,NULL,101,c("jeffreys"),1000,NULL,"methodcol","referencecol",c(0.5,0.5,1,1),c(-3,-4,-5,-6),,,)
...
save(asthma23pat, file="data/asthma.RData",version=2) 

save(acupuncture, file="data/acupuncture.RData",version=2)
...
Sys.setenv(PATH = paste(Sys.getenv("PATH"),"C:\\Users\\rmjlkm0\\AppData\\Local\\Programs\\MiKTeX\\miktex\\bin\\x64", sep=.Platform$path.sep))
sessioninfo::package_info("mimix")
devtools::build_manual(pkg = "mimix",path=".")
#Did you accidentally source a file rather than using `load_all()`?
rm(list = c("AddDelta", "analyselist", "Causal_loop", "CIR_loop", "fillinterims", "getimpdatasets", "ifmethodindiv", "LMCF_loop", "mimix", "pass2Loop", "preprodata", "preproIndivdata"))`
detach("package:mimix",unload=TRUE)


#try recoding trewat 2,3 into 1,2,...
# should work whether treatvar numeric or char    
asthma23pat<-read.csv(file='N:/Documents/asthma23pat.csv')
asthmat<-read.csv(file='N:/Documents/asthma.csv')

clus<-factor(unlist(asthma23$treat))         
previous_levels_clus <- levels(clus)
levels(clus) <- 1:(nlevels(clus))
clus<-as.numeric(as.character(clus))
clus<-factor(impdataset[,treatvar])

ytest<-unique(asthma23[,c(treatvar,idvar)])
tmpdata<-merge(impdata, interims ,by.x=  ".id",by.y= idvar)
tmpdata<-merge(impdataset,unique(asthma23[,c(treatvar,idvar)]) ,by.x= paste0(".",idvar),by.y= idvar)

# to use value labels !
impdataset[,treatvar] <- ordered(impdataset[,treatvar],labels=prev_levels)

if (impdataset$.id %in% ytest$id) {
  impdataset[,treatvar]<-ytest[,treatvar]
}

xtest<-impdataset[,!(names(impdataset) %in% c(treatvar))]
xtmpdata<-merge(xtest,,by.x= paste0(".",idvar),by.y= idvar)
impdataset$.id %in% ytest$id, 
xytest<-merge(xtest,unique(asthma23[,c(treatvar,idvar)]),by.x=".id",by.y=idvar)
# then move id co lto last


testquote<- function(data,covar=NULL) {
  ...
  # attempt to alleviate need for quotes in input arguments
  browser()
...
# to build pdf ref manual
devtools::build_manual(pkg=find.package(("mimix")),path=getwd())

widetolong<- function(data,depvar,treatvar,idvar,timevar)
{
  browser(text="1303")
  #find timepoints from depvar 
  # get("data")[,grepl(depvar,colnames(get("data")))]
  varyingnames<-names(get("data")[,grepl(deparse(substitute(depvar)),colnames(get("data")))])
  testout<- reshape(get("data"),varying = varyingnames ,direction="long",sep=".")
  testout<-(testout[,-c(ncol(testout))])
  
  # then need to merge with input data
  
  testmerge<-(merge(acupuncture,testout,by.x = c("id","time"),by.y = c(".id","time")))
  tail(testmerge[order(testmerge[,".imp"],testmerge$time),])
  
  return(testout)
}

asthma$fev<-ifelse (asthma$id==5099,NA,asthma$fev)
subset(asthmaLMCF, .id==5099)
# add 10 to base
asthma$base<- asthma$base+10




sts4dummy[,grepl(depvar,names(sts4dummy))],MARGIN=2,function(x) ifelse(!is.na(x),0,1))
  merge(impdata, interims ,by.x=  ".id",by.y= idvar)
  ...
  impdatamerge<-(merge(asthma23pat,impdatalong,by.x = c(idvar,timevar),by.y = c(".id",timevar)))
  # can delete all .x's 
  ...
  # quest is whether to create .id var?
  ############end###
  
  ## for indiv case
  impdatalong<- reshape(impdataset,varying = varyingnames ,direction="long",sep=".",v.names = depvar)

  mpantiCausal_2 <- mimix(data=antidepressant,covar=c(basval,PATIENT.SEX),depvar=HAMD17.TOTAL,treat=TREATMENT.NAME,idvar=PATIENT.NUMBER,timevar=VISIT.NUMBER,M=10,reference=1,method=NULL,seed=101)
  
  impdatasetJ2Rdelta<-(mimix(data=asthma23pat,covar=c(base),depvar=fev,treatvar=treatchr,idvar=patient,timevar=time,M=10,reference=1,method=J2R,seed=101,delta=c(1,5,2,3),K0=1,K1=1 ) )
  
  fit<-with(data= as.mids(subset(impacupLMCF[[2]],time==12)),  lm(head~treat+head_base))
  
  impacupLMCF <- mimix(data=acupuncture,covar=c("head_base","sex"),depvar=head,treatvar=treat,idvar=id,timevar=time,method=LMCF,reference=1,K0=1,K1=0.5,M=5,seed=54321)
