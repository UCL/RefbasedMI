...
AddDelta<-function(vec_tst,covar,mata_imp,delta,dlag)  {
 # infinite loop ?browser(text="1712")
  ...
  onezero<- sapply(setdiff(vec_tst,covar), function(x) return(mata_imp[1,paste0(x,".miss")]))
  ...
  # for (v in lastVisit:length(onezero)) {
   ...
   # needs be inserted here as prvious passed by argument
  ncovar<- length(covar)
